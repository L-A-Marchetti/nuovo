...
app::app(std::string title, int w, int h) : title(title), w(w), h(h), r(nullptr)
{
	...
	int chans = 0;
	this->modules.push_back(new module("BASS DRUM", {{"TUNE", 32}, {"LEVEL", 64}, {"ATTACK", 45}, {"DECAY", 100}}, "kick.wav", ++chans));
	this->modules.push_back(new module("SNARE DRUM", {{"TUNE", 20}, {"LEVEL", 73}, {"TONE", 50}, {"SNAPPY", 75}}, "snare.wav", ++chans));
	this->modules.push_back(new module("LOW TOM", {{"TUNE", 40}, {"LEVEL", 127}, {"DECAY", 60}}, "low_tom.wav", ++chans));
	this->modules.push_back(new module("MID TOM", {{"TUNE", 35}, {"LEVEL", 127}, {"DECAY", 70}}, "mid_tom.wav", ++chans));
